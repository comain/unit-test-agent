...
def _client(checks_client: GithubChecksClient | None = None, secret: str = SECRET):
    service = ApiTriggerService(
        protocols=ProtocolRegistry(
            [RdcProtocol(), GithubWebhookProtocol(webhook_secret=secret, checks_client=checks_client)]
        )
    )
...
def test_parse_trigger_maps_pull_request_fields():
    ...
    assert request.jira_id == "TASK-100"  # inferred from the PR title
