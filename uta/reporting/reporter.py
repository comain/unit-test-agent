"""Build and render language-neutral generated-test reports."""
...
def _test_quality_warning_count(res: Dict[str, Any]) -> int:
    quality = res.get("testQuality")
    if not isinstance(quality, dict):
        return 0
    return int(quality.get("warningCount") or 0)


def _test_quality_top_rule(res: Dict[str, Any]) -> Optional[str]:
    quality = res.get("testQuality")
    if not isinstance(quality, dict):
        return None
    top_rule_ids = quality.get("topRuleIds") or []
    first = top_rule_ids[0] if top_rule_ids and isinstance(top_rule_ids[0], dict) else {}
    return str(first.get("ruleId")) if first.get("ruleId") else None
            ...
            file_metrics.append(
                {
                    ...
                    "test_quality_warning_count": _test_quality_warning_count(res),
                    "test_quality_top_rule": _test_quality_top_rule(res),
                }
            )
