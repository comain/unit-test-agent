...
MISSING_PLUGIN_SUMMARY_PREFIX = "Missing UTA test-enforcement plugin/profile"
...
def can_create_fix_session(record: CiTaskRecord) -> bool:
    ...
    if isinstance(evidence, dict) and evidence.get("repairEligible") is False:
        return False
...
def _is_missing_plugin_or_profile(record: CiTaskRecord) -> bool:
    ...
    tooling = evidence.get("tooling") if isinstance(evidence, dict) else {}
    if isinstance(tooling, dict) and tooling.get("available") is False:
        return True
    return (
