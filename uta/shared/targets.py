...
class RawTargetSelection:
    ...
    A `TargetNormalizer` turns this permissive shape into a strict TargetRef.
    """
...
def coerce_target(value: Union[str, Mapping[str, Any], TargetIdentity]) -> TargetIdentity:
    ...
    if isinstance(value, str):
        if value.startswith(("pyfile:", "pysymbol:")):
            # Only the Python target normalizer, resolved by name. Building the
            # whole registry here imported the Java backend as well, to read a
            # string prefix that had already said which language it was.
            from uta.shared.backends import make_backend

            normalizer = make_backend("python", "adapter")
            return normalizer.normalize_target(RawTargetSelection(target=value))
