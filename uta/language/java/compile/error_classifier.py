...
# Maven stamps a severity on every line it emits. `[WARNING]` carries the same
# `path.java:[line,col] msg` shape as `[ERROR]`, so without this the classifier
# reads javac warnings as compile failures -- which reached a passing report as
# a "test compilation blocker" and can send fix loops chasing nothing.
_SEVERITY_PREFIX_RE = re.compile(r"^\s*\[(?P<level>[A-Z]+)\]")

# Raw javac lines reach us without a Maven severity (stderr, forked compilers).
# These messages are only ever emitted as warnings/notes.
_NON_ERROR_MESSAGE_RE = re.compile(
    r"(?:^|\s)(?:warning|note):"
    r"|is internal proprietary API"
    r"|may be removed in a future release"
    r"|uses unchecked or unsafe operations"
    r"|deprecation: "
    r"|[Rr]ecompile with -Xlint",
)


def _is_error_line(line: str, message: str) -> bool:
    """True when a matched location line reports an error rather than a warning."""
    prefix = _SEVERITY_PREFIX_RE.match(line)
    if prefix:
        # Maven's own severity is authoritative: a project compiling with
        # -Werror legitimately reports deprecation as `[ERROR]`.
        return prefix.group("level") == "ERROR"
    return not _NON_ERROR_MESSAGE_RE.search(message)
...
def classify_compile_errors(raw: str) -> List[CompileError]:
    ...
    while i < len(lines):
        line = lines[i]
        # `_LOC_RE`'s path class matches spaces and dots, so on a line with no
        # ".java" at all it backtracks from every start position -- quadratic in
        # the line length (a 16KB line costs ~0.4s on its own). Maven logs are
        # mostly such lines, so this substring check is what keeps parsing a
        # full build log linear.
        if ".java" not in line:
            i += 1
            continue
        ...
        if not _is_error_line(line, msg):
            i += 1
            continue
