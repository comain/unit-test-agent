...
from uta.enforcement.mutation_repair import MutationRepairContext, MutationRepairGroup
...
def parse_pitest_mutations(xml_path: str) -> List[Dict[str, Any]]:
    """Every `<mutation>` row of a PIT report, whatever its status.

    `parse_pitest_report` keeps only survivors of one class, which is right for
    a repair prompt. Deciding what a gate counted needs the whole report: the
    killed rows are what the survivors are measured against.
    """
    ...
    for mutation in root.findall("mutation"):
        ...
        rows.append({
            ...
            "mutated_class": text("mutatedClass"),
            "method": text("mutatedMethod"),
            "method_description": text("methodDescription"),
            "line": int(line) if line.isdigit() else 0,
            "mutator": text("mutator"),
            "indexes": ",".join((node.text or "").strip() for node in mutation.findall("indexes/index")),
            "blocks": ",".join((node.text or "").strip() for node in mutation.findall("blocks/block")),
...
def pit_families_to_repair_groups(families: List[Dict[str, Any]]) -> List[MutationRepairGroup]:
    """Adapt ``summarize_surviving_mutants`` dicts onto the shared model.

    The shared ``MutationRepairGroup`` (uta.enforcement.mutation_repair) is the
    language-neutral carrier for mutation-repair state; this proves it losslessly
    holds everything PIT surfaces (mutator, killability, ROI, priority flags, and
    per-example survivors). The per-mutant examples become ``survivors`` so the
    Java renderer can read them back from the model. ``roi``/``effort_*`` are set
    only when ``score_families`` has annotated the family (ROI enabled).
    """
    groups: List[MutationRepairGroup] = []
    for family in families:
        has_roi = "roi" in family
        groups.append(
            MutationRepairGroup(
                symbol=family.get("method", "(unknown)"),
                count=int(family.get("count", 0) or 0),
                lines=tuple(family.get("lines", [])),
                survivors=tuple(family.get("examples", [])),
                family=family.get("family", "other"),
                killability=family.get("killability", "low"),
                mutator=family.get("mutator", ""),
                effort_score=str(family.get("effort_score", "")) if has_roi else "",
                effort_band=str(family.get("effort_band", "")) if has_roi else "",
                roi=float(family.get("roi", 0.0)) if has_roi else None,
...
def java_pit_families_to_repair_context(
    *,
    target_id: str,
    source_path: str,
    ...
    """Adapt Java PIT survivor families into the shared planner contract."""

    groups = pit_families_to_repair_groups(families)
    return MutationRepairContext(
        target_id=target_id,
        source_path=source_path,
        test_paths=test_paths,
        reproduce_command=reproduce_command,
        survivor_count=sum(group.count for group in groups),
        groups=tuple(groups),
...
def format_mutation_families_markdown(families: List[Dict[str, Any]]) -> str:
    """Render the Java mutation-family summary from the shared model.

    Java keeps its own compact one-line-per-family wording (prompt text stays
    per-language), but the data now flows through ``MutationRepairGroup`` rather
    than ad-hoc dicts. Output is byte-identical to the previous dict renderer.
    """
    if not families:
        return "- No surviving mutant families were parsed.\n"

    groups = pit_families_to_repair_groups(families)
    has_roi = any(group.roi is not None for group in groups)
    lines: List[str] = []
    if has_roi:
        lines.append("_Ranked by kill-per-effort ROI — tackle the top entry first._")
        lines.append("")
    for group in groups:
        line_list = ", ".join(str(line) for line in group.lines[:8]) or "unknown"
        ...
        roi_suffix = ""
        if has_roi:
            roi_value = group.roi if group.roi is not None else 0.0
            roi_suffix = (
                f", effort {group.effort_score or '?'} ({group.effort_band or '?'}),"
                f" roi {roi_value}"
            )
        lines.append(
            f"- method `{group.symbol}` — `{group.family}` via `{group.mutator}` — {group.count} survivor(s), "
            f"killability `{group.killability}`{roi_suffix}, {priority_note}, line(s): {line_list}"
        )
        for example in group.survivors:
            detail = example.get("detail", "")
            suffix = f" — {detail}" if detail else ""
            lines.append(
                f"  - line {int(example.get('line', 0) or 0)} in `{example.get('method', '') or group.symbol}`: "
