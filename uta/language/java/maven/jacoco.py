...
def _resolve_jacoco_agent_jar() -> Optional[str]:
    ...
    for repo_dir in (home / ".m2" / "repository", Path("/home/user/internal_repository")):
...
def _run_maven_jacoco(
    ...
    # Immediately before the run, not once at setup: a pinned `skipTests`
    # restored mid-run turns the next measurement into a silent zero.
    relax_surefire_skiptests(repo_path)
