...
def preseed_compile_context(
    ...
    *,
    ...
    # Build pseudo-candidates markdown table from saved FQNs
    md_lines: List[str] = []
    for short_name, fqns in sorted(resolved.items()):
        md_lines.append(f"### `{short_name}`\n")
        md_lines.append("| Candidate FQN | Source | Locality |")
        md_lines.append("|---|---|---|")
        for fqn in fqns:
            md_lines.append(f"| `{fqn}` | (prior run) | other |")
        md_lines.append("")
    md_block = "\n".join(md_lines).strip()
    ...
    facts = [
        f"Prior-run resolution: `{name}` → " + ", ".join(f"`{fqn}`" for fqn in fqns)
        for name, fqns in resolved.items()
        if fqns
    ]
    if facts:
